      ...
      {(movie && credits)  && (
        <div>
              ...
              <div className=" flex w-full sm:w-2/3 mr-32 mt-10 align-middle justify-center">
                <div className=" flex flex-col  justify-center align-middle mx-4">
                  ...
                  <Typography level="body-sm" textColor={"gray"}>
                    {movie.genres
                      .map((genre) => {
                        const genreName = genres_ids.find((g) => g.id === genre.id);
                        return genreName.name;
                      })
                      .join(", ")}
                  </Typography>
                  <p className=" mt-4 text-white">{movie.overview}</p>
                  <p className=" mt-4 text-white">Runtime: <strong className=" text-gray-300">{movie.runtime}</strong> min</p>
            ...
            <div className=" flex flex-col mt-10 m-32">
                ...
                <div className=" grid grid-rows-5  sm:grid-cols-5   gap-10">
                    {credits.cast.slice(0, 5).map((actor) => (
                    <div key={actor.id} className=" flex flex-col ml-4 w-full h-auto">
                        <img
